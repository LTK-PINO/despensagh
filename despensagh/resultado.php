...
<head>
    ...
    <title>Resultat | Despensa García Haro</title>
...
<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Verifica si se ha enviado algún término de búsqueda
        if (isset($_POST['buscar']) && !empty($_POST['buscar'])) {
            // Escapa los caracteres especiales para evitar inyecciones de SQL
            $busqueda = htmlspecialchars($_POST['buscar']);

            // Realizar la consulta para obtener los datos del usuario
            $query = "SELECT id_producto, Nombre_producto, Precio_Venta, Imagen FROM productos WHERE Nombre_producto LIKE '%$busqueda%' OR Categoria LIKE '%$busqueda%'";
            $result = mysqli_query($conn, $query);
            if ($result) {
                if (mysqli_num_rows($result) > 0) {
                    $row = mysqli_fetch_assoc($result); 
                    ...
                    echo "
                    <h1 class='h1-busqueda'>Resultat de la cerca $busqueda</h1>
                            ...
                            <div class='contenedor-busqueda'>
                                <div class='box-busqueda'>
                                    ...
                                    <div class='boton-cesta'>
                                        <button onclick=\"location.href='php/producte.php?id=$id'\" type='button'>Veure Producte <span class='material-symbols-outlined icono-botones'>visibility</span></button></a>
                                    </div>
