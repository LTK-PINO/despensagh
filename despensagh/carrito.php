...
<body>
    ...
    <div class="contenedor-carrito">
        <div class="contenido-carrito">
            ...
            <div class="carrito-interno">
            <?php 
                // Consulta para obtener los productos en el carrito
                $query_carrito = "SELECT * FROM carrito WHERE Usuario = ?";
                $stmt_carrito = $conn->prepare($query_carrito);
                $stmt_carrito->bind_param("s", $Usuari);
                ...
                if ($resultado_carrito->num_rows > 0) {
                    while ($row_carrito = $resultado_carrito->fetch_assoc()) {
                        $Nombre_Producto = $row_carrito['Nombre_Producto'];
                        $ImagenProducto = $row_carrito['Imagen'];
                        $encodeImage = base64_encode($ImagenProducto);
                        echo "
                            <div class='imagen'>
                                <img src='data:image/png;base64," . $encodeImage . "' alt='$Nombre_Producto'>
                            </div>
                            ";
                    }
                } else {
